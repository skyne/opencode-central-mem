...
export class LocalMemoryClient {
  ...
  async searchMemories(query: string, containerTag: string, scope: MemoryScope = "project"): Promise<{ success: boolean; results: any[]; total: number; timing: number; error?: string }> {
    try {
      await this.initialize();

      if (syncManager.isConnected() && scope !== "all-projects") {
        return syncManager.searchBoth(query, containerTag, scope);
      }
  ...
  async addMemory(
    ...
    try {
      ...
      if (sync_to_central) {
        (dynamicMetadata as any)._syncStatus = 'pending-upload';
      }
      ...
      if (sync_to_central || (metadata as any)?._syncStatus === 'pending-upload') {
        syncManager.queueUpload(id);
      }
    ...
    try {
      ...
      for (const shard of allShards) {
        ...
        if (memory) {
          ...
          if (meta._syncStatus === 'synced' || meta._centralId) {
            syncManager.queueDelete(memoryId);
          }
